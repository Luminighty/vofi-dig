...
export class Game {
	...
	initMap() {
		...
		for (let i = 0; i < cave.length; i++) {
			this.createEntity("Tile", { x: i * gridSize, y: -gridSize });
			this.createEntity("Tile", { y: i * gridSize, x: -gridSize });
			this.createEntity("Tile", { x: i * gridSize, y: cave.length * gridSize });
			this.createEntity("Tile", { x: cave[i].length * gridSize, y: i * gridSize });
		}
	}
